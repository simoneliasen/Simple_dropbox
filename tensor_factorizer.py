...
class TensorFactorizer:
    def __init__(
        self, model_name, params, loss_function="margin", dataset="wn18", save_dir=None
    ):
    ...
    def load_session(self, itr):
        self.loader.restore(
            self.sess,
            self.model_name + "_weights/" + self.dataset + "/" + itr + ".ckpt",
        )
    ...
    def create_optimizer(self):
        if self.loss_function == "margin":
            self.loss = (
                tf.reduce_sum(
                    tf.nn.relu(self.params.gamma + self.pos_dissims - self.neg_dissims)
                )
                + self.params.alpha * self.regularizer
            )
        else:
            self.loss = (
                tf.reduce_sum(tf.nn.softplus(-self.labels * self.scores))
                + self.params.alpha * self.regularizer
            )

        self.optimizer = tf.train.AdagradOptimizer(self.params.learning_rate).minimize(
            self.loss
        )
    ...
    def save_model(self, itr):
        if self.save_dir:
            ...
            filename = self.model_name + "_weights/" + self.dataset + "/"

        filename = filename + str(itr) + ".ckpt"
        ...
        for itr in range(1, self.params.max_iterate + 1):
            ...
            for b in range(self.num_batch):
                if self.loss_function == "margin":
                    ...
                    h, r, t, y = self.reader.next_batch(
                        format="triple_label", neg_ratio=self.params.neg_ratio
                    )
                    _, err = self.sess.run(
                        [self.optimizer, self.loss],
                        feed_dict={self.head: h, self.rel: r, self.tail: t, self.y: y},
                    )
                total_loss += err
    ...
    def test(self, triples):
        ...
        for i, triple in enumerate(triples):
            ...
            head_raw_preds = self.sess.run(
                self.dissims,
                feed_dict={
                    self.head: head_raw_h,
                    self.rel: head_raw_r,
                    self.tail: head_raw_t,
                },
            )
            tail_raw_preds = self.sess.run(
                self.dissims,
                feed_dict={
                    self.head: tail_raw_h,
                    self.rel: tail_raw_r,
                    self.tail: tail_raw_t,
                },
            )
            head_fil_preds = self.sess.run(
                self.dissims,
                feed_dict={
                    self.head: head_fil_h,
                    self.rel: head_fil_r,
                    self.tail: head_fil_t,
                },
            )
            tail_fil_preds = self.sess.run(
                self.dissims,
                feed_dict={
                    self.head: tail_fil_h,
                    self.rel: tail_fil_r,
                    self.tail: tail_fil_t,
                },
            )
